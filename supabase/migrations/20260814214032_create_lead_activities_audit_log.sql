/*
# Lead Activity Log — persistent audit trail for lead mutations

## Overview
Creates a `lead_activities` table that records every meaningful change to a lead,
plus two SECURITY DEFINER RPC functions that atomically update a lead and create
activity records for each changed field.

## New Tables
### lead_activities
- id (uuid, PK, default gen_random_uuid())
- lead_id (text, NOT NULL, FK → lead_entries.id ON DELETE CASCADE)
- actor_user_id (uuid, nullable — auth.users.id of the person who acted)
- actor_rep_code (text, nullable — sales_representatives.rep_code)
- actor_name (text, nullable — denormalized rep name for display)
- action_type (text, NOT NULL — e.g. STATUS_CHANGED, FIELD_UPDATED, SALES_REP_CHANGED, EVENT_CHANGED, LEAD_TEMPERATURE_CHANGED, REVIEWED, CREATED)
- field_name (text, nullable — which column changed)
...
## Security (RLS)
- RLS enabled on lead_activities.
- SELECT: authenticated users can read activities for leads they are authorized
  to view (admins see all; sales_reps see activities for their own leads only).
- INSERT: NO direct client INSERT policy — all inserts go through SECURITY DEFINER
  functions, which derive actor identity from auth.uid() server-side.
- UPDATE: NO policy — activity records are immutable once created.
- DELETE: NO policy — activity records cannot be deleted by clients.

## Functions
### _format_activity_value(p_val jsonb) → text
Helper that formats jsonb values for human-readable display:
- null → "Not set"
- arrays → comma-separated elements
- empty strings → "Not set"
- everything else → scalar text value

### update_lead_with_audit(p_lead_id text, p_updates jsonb) → jsonb
SECURITY DEFINER. Atomically:
1. Derives actor identity from auth.uid() → sales_representatives (server-side, not trusted from client)
2. Locks and reads the current lead row (FOR UPDATE)
3. Validates authorization (sales_rep can only update own leads)
4. Applies the UPDATE to lead_entries
5. Diffs all tracked fields between old and new values
6. Creates one activity record per actually-changed field
7. Returns { lead: updated_row_jsonb, activities_created: count }

Special fields with dedicated action types:
- lead_status → STATUS_CHANGED (with display label mapping, e.g. QUALIFIED → "Samples Sent")
- sales_rep_code → SALES_REP_CHANGED (with rep name lookup)
- event_code → EVENT_CHANGED (with event name lookup)
- lead_temperature → LEAD_TEMPERATURE_CHANGED
- is_reviewed → REVIEWED (only when changing false → true)

Generic text/array fields → FIELD_UPDATED:
- client_name, designation, company, address, state, notes, website,
  application, price_range, quick_keywords, target_market, certification,
  benchmark, phones, emails

### log_lead_created(p_lead_id text) → void
SECURITY DEFINER. Creates a CREATED activity for a newly promoted lead.
Actor derived from auth.uid() → sales_representatives. Silently skips if no
rep profile is found (non-blocking for the capture promotion flow).

## Index
- (lead_id, created_at DESC) for efficient pagination of activities per lead.
*/

-- ═══ 1. Helper function ═══════════════════════════════════════════════════════

CREATE OR REPLACE FUNCTION _format_activity_value(p_val jsonb) RETURNS text AS $$
BEGIN
  IF p_val IS NULL OR p_val::text = 'null' THEN
    RETURN 'Not set';
  END IF;
  IF jsonb_typeof(p_val) = 'array' THEN
    RETURN COALESCE(array_to_string(ARRAY(SELECT jsonb_array_elements_text(p_val)), ', '), 'Not set');
  END IF;
  IF jsonb_typeof(p_val) = 'string' AND p_val #>> '{}' = '' THEN
    RETURN 'Not set';
  END IF;
  RETURN p_val #>> '{}';
END;
...
CREATE TABLE IF NOT EXISTS lead_activities (
  ...
  lead_id         text        NOT NULL REFERENCES lead_entries(id) ON DELETE CASCADE,
  actor_user_id   uuid,
  actor_rep_code  text,
  actor_name      text,
...
-- ═══ 4. SELECT policy — admins see all, sales_reps see their own leads' activities ══

DROP POLICY IF EXISTS "select_lead_activities" ON lead_activities;
CREATE POLICY "select_lead_activities" ON lead_activities FOR SELECT
TO authenticated
USING (
  EXISTS (
    ...
    AND (
      ...
      OR EXISTS (
        ...
        WHERE le.id = lead_activities.lead_id
        AND le.sales_rep_code = sr.rep_code
      )
    )
  )
);

-- No INSERT, UPDATE, or DELETE policies — activity records are immutable and
-- can only be created by the SECURITY DEFINER functions below.

-- ═══ 5. Index for pagination ════════════════════════════════════════════════════

CREATE INDEX IF NOT EXISTS idx_lead_activities_lead_id_created_at
ON lead_activities (lead_id, created_at DESC);

-- ═══ 6. update_lead_with_audit — atomic update + audit ═════════════════════════
...
BEGIN
  ...
  UPDATE lead_entries SET
    ...
    phones           = CASE WHEN p_updates ? 'phones'           THEN (p_updates->'phones')::text[]             ELSE phones END,
    emails           = CASE WHEN p_updates ? 'emails'           THEN (p_updates->'emails')::text[]             ELSE emails END,
...
-- ═══ 7. log_lead_created — for capture promotion flow ═══════════════════════════

CREATE OR REPLACE FUNCTION log_lead_created(p_lead_id text)
RETURNS void
SECURITY DEFINER
SET search_path = public
AS $$
DECLARE
  v_actor record;
BEGIN
  SELECT sr.auth_user_id, sr.rep_code, sr.name
  INTO v_actor
  FROM sales_representatives sr
  WHERE sr.auth_user_id = auth.uid();
  ...
  INSERT INTO lead_activities (lead_id, actor_user_id, actor_rep_code, actor_name, action_type, field_name, old_value, new_value, note)
  VALUES (p_lead_id, v_actor.auth_user_id, v_actor.rep_code, v_actor.name, 'CREATED', null, null, null, 'Lead created');
END;
$$ LANGUAGE plpgsql;