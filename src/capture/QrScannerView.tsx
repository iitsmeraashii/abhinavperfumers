...
import { X, RefreshCw, AlertCircle, CheckCircle2, Loader2, QrCode, Link, FileText } from 'lucide-react';
...
// ── Success content helpers ───────────────────────────────────────────────────

function getSuccessLabel(result: ParsedContact): string {
  if (result.hasData) return 'Contact details extracted';
  switch (result.qrType as QrContentType) {
    case 'url':       return 'Website link captured';
    case 'plaintext': return 'Text information captured';
    default:          return 'QR scanned — no structured data found';
  }
}
...
function getSuccessSubtext(result: ParsedContact): string | null {
  ...
  switch (result.qrType as QrContentType) {
    ...
    case 'plaintext': return 'Raw text has been saved.';
    default:          return 'You can add details manually.';
  }
}

function successRingColor(qrType: QrContentType, hasData: boolean): string {
  if (hasData) return 'bg-teal-500/20';
  if (qrType === 'url') return 'bg-sky-500/20';
  return 'bg-stone-500/20';
}

function SuccessIcon({ qrType, hasData }: { qrType: QrContentType; hasData: boolean }) {
  if (hasData) return <CheckCircle2 className="w-8 h-8 text-teal-400" />;
  if (qrType === 'url') return <Link className="w-8 h-8 text-sky-400" />;
  return <FileText className="w-8 h-8 text-stone-300" />;
}
...
export function QrScannerView({ onScanned, onCancel }: Props) {
  ...
  useEffect(() => {
    ...
    // Small delay lets the DOM element finish mounting before the library attaches
    const t = setTimeout(() => startScanner(elementId), 80);
    return () => clearTimeout(t);
  }, [elementId, startScanner]);

  // Bubble result up after a brief success pause so the user sees the overlay
  useEffect(() => {
  ...
  return (
    <div className="mt-4 flex flex-col" style={{ minHeight: 'calc(100dvh - 240px)' }}>
      ...
      <div className="relative flex-1 rounded-2xl overflow-hidden bg-stone-950 min-h-[280px]">
        ...
        {/*
         * html5-qrcode mounts <video> + its own UI here.
         * The global CSS in index.css strips the library's default chrome.
         */}
        <div
          id={elementId}
        ...
        {/* ── Scan guide overlay (teal corner brackets + vignette) ── */}
        {isScanning && (
          <div className="absolute inset-0 flex items-center justify-center pointer-events-none z-10">
            <div className="relative w-52 h-52">
              {/* Dim vignette outside the frame */}
              <div
                className="absolute -inset-[9999px] bg-black/40"
                style={{ boxShadow: '0 0 0 9999px rgba(0,0,0,0.45)' }}
              />
              {/* Corner brackets — match the 200×200 qrbox */}
        ...
        {/* ── Success overlay ── */}
        {isSuccess && result && (
          <div className="absolute inset-0 flex flex-col items-center justify-center gap-3 bg-stone-950/85 z-20 animate-in fade-in duration-150 px-8 text-center">
            <div className={`w-16 h-16 rounded-full ${successRingColor(result.qrType, result.hasData)} flex items-center justify-center animate-in zoom-in-75 duration-200`}>
              <SuccessIcon qrType={result.qrType} hasData={result.hasData} />
            </div>
            <p className="text-sm font-semibold text-white">
              {getSuccessLabel(result)}
            </p>
