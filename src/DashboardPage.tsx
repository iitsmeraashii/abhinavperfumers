...
const COLOR_MAP = {
  blue:   { bg: 'bg-blue-50',   border: 'border-blue-100',   icon: 'text-blue-500',   val: 'text-blue-700',   sub: 'text-blue-400' },
  green:  { bg: 'bg-green-50',  border: 'border-green-100',  icon: 'text-green-500',  val: 'text-green-700',  sub: 'text-green-400' },
  red:    { bg: 'bg-red-50',    border: 'border-red-100',    icon: 'text-red-500',    val: 'text-red-700',    sub: 'text-red-400' },
  yellow: { bg: 'bg-yellow-50', border: 'border-yellow-100', icon: 'text-yellow-500', val: 'text-yellow-700', sub: 'text-yellow-400' },
  stone:  { bg: 'bg-stone-50',  border: 'border-stone-100',  icon: 'text-stone-400',  val: 'text-stone-700',  sub: 'text-stone-400' },
};

function KpiCard({ icon, label, value, color, sub }: KpiCardProps) {
  const c = COLOR_MAP[color];
  return (
    <div className={`rounded-xl border ${c.bg} ${c.border} px-5 py-4 flex items-start gap-4`}>
...
function FunnelStep({ label, value, total, color, bar, isLast }: FunnelStepProps) {
          ...
          <section>
            ...
            <div className="bg-white border border-stone-200 rounded-xl px-6 py-5">
              <div className="flex flex-col sm:flex-row gap-6">
                ...
                <FunnelStep label="Qualified" value={data.qualified_leads} total={funnelTotal} color="text-violet-600" bar="bg-violet-400" />
