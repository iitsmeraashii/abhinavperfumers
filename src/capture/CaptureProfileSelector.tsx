...
interface Props {
  ...
  disabled?: boolean;
  /** Optional heading shown above the toggle. */
  label?:      string;
  /** Optional helper text shown under the toggle. */
  helperText?: string;
}
...
export function CaptureProfileSelector({ value, onChange, disabled = false, label, helperText }: Props) {
  return (
    <div>
      {label && (
        <div className="mb-1.5 text-sm font-semibold text-stone-700">{label}</div>
      )}
      <div className={`flex items-center gap-1 p-1 bg-stone-100 rounded-xl transition-opacity ${disabled ? 'opacity-40 pointer-events-none' : ''}`}
        role="radiogroup"
        aria-label={label ?? 'Capture profile'}
      >
        {OPTIONS.map((opt) => {
      ...
      {helperText && (
        <p className="mt-1.5 text-xs text-stone-400 leading-relaxed">{helperText}</p>
      )}
    </div>
  );
}
